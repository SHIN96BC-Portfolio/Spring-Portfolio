...
CREATE TABLE static_page (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug                VARCHAR(100) NOT NULL UNIQUE,  -- 예: about, terms
    ...
    CONSTRAINT uq_static_page_page_key UNIQUE (page_key),
...
COMMENT ON COLUMN static_page.slug IS 'URL 경로 식별자. UNIQUE. 예: about, terms';
...
CREATE TABLE home_section (
    ...
    CONSTRAINT uq_home_section_page_section_key
        UNIQUE (page_key, section_key),
    CONSTRAINT uq_home_section_page_display_order
...
CREATE INDEX idx_home_section_page_active_order
    ON home_section (page_key, display_order)
    WHERE is_active;
...
COMMENT ON COLUMN home_section.section_key IS
    'page_key 내 고유 슬롯 ID. 예: hero, project-grid (UNIQUE with page_key)';
...
-- 활성 메뉴: 부모별 정렬
-- [LOW-CM-5] navigation_menu 도 starts_at/ends_at 은 소량일 때 앱 필터 (banner 와 동일)
CREATE INDEX idx_navigation_menu_parent_active_order
    ON navigation_menu (parent_id, display_order)
    WHERE is_active;

-- 활성 배너: 슬롯별 정렬
-- [LOW-CM-5] starts_at/ends_at "지금 노출"은 소량 데이터에서 앱 WHERE 로 충분.
--   트래픽 증가 시 기간 조건을 포함한 partial index 검토.
CREATE INDEX idx_banner_slot_active_order
    ON banner (slot, display_order)
    WHERE is_active;

-- 게시된 정적 페이지: 최신 게시순
CREATE INDEX idx_static_page_published_at
    ON static_page (published_at DESC)
    WHERE is_published;

-- 활성 섹션: 페이지별·게시 스냅샷 있는 행
CREATE INDEX idx_home_section_page_type_active_order
    ON home_section (page_key, section_type, display_order)
